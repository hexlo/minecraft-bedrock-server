...
RUN apt update && apt install -y curl unzip nano wget python3
...
RUN export DOWNLOADED_VERSION=$(python3 ${SERVER_DIR}/scripts/download_latest_version.py --dest ${SERVER_DIR} --version "${VERSION}") && \
